...
// NOTE. EVERYTHING SHOULD BE IN FIXED POINT 20.12!

#define TO_20_12(a) ((int32)((1<<12)*(a)))
#define FROM_20_12(a) ((a)>>12)
...
/*@null@*/ static unsigned *precalculate_heights(unsigned maxdist) {
    ...
    if (result) {
        for (i = 0; i <= maxdist; ++i) {
            result[i] = (unsigned)FROM_20_12(sqrtf32(TO_20_12(maxdist*maxdist - i*i)));
        }
        result[maxdist+1] = 0;
    }
    return result;
...
#define FOV_DEFINE_OCTANT(signx, signy, rx, ry, nx, ny, nf, apply_edge, apply_diag)             \
    static void fov_octant_##nx##ny##nf(                                                        \
        ...
        case FOV_SHAPE_CIRCLE:                                                                  \
            h = (unsigned)FROM_20_12(sqrtf32(TO_20_12(data->radius*data->radius - dx*dx)));     \
            break;                                                                              \
        ...
        for (dy = dy0; dy <= dy1; ++dy) {                                                       \
            ...
            if (settings->opaque(data->map, x, y)) {                                            \
                ...
                if (prev_blocked == 0) {                                                        \
                    end_slope_next = fov_slope(TO_20_12(dx) + TO_20_12(0.5f), TO_20_12(dy) - TO_20_12(0.5f));\
                ...
                if (prev_blocked == 1) {                                                        \
                    start_slope = fov_slope(TO_20_12(dx) - TO_20_12(0.5f), TO_20_12(dy) - TO_20_12(0.5f));\
                }                                                                               \
                prev_blocked = 0;                                                               \
            }                                                                                   \
        }                                                                                       \
...
static void _fov_circle(fov_private_data_type *data) {
     ...
     *   /mmy|mpy\
     */
    fov_octant_ppn(data, 1, TO_20_12(0.0f), TO_20_12(1.0f));
    fov_octant_ppy(data, 1, TO_20_12(0.0f), TO_20_12(1.0f));
    fov_octant_pmn(data, 1, TO_20_12(0.0f), TO_20_12(1.0f));
    fov_octant_pmy(data, 1, TO_20_12(0.0f), TO_20_12(1.0f));
    fov_octant_mpn(data, 1, TO_20_12(0.0f), TO_20_12(1.0f));
    fov_octant_mpy(data, 1, TO_20_12(0.0f), TO_20_12(1.0f));
    fov_octant_mmn(data, 1, TO_20_12(0.0f), TO_20_12(1.0f));
    fov_octant_mmy(data, 1, TO_20_12(0.0f), TO_20_12(1.0f));
}
 ...
 */
static int32 betweenf(int32 x, int32 a, int32 b) {
    if (x < a) {
        return a;
    } else if (x > b) {
        return b;
...
#define BEAM_DIRECTION(d, p1, p2, p3, p4, p5, p6, p7, p8)   \
    if (direction == d) {                                   \
        end_slope = betweenf(a, TO_20_12(0.0f), TO_20_12(1.0f));                \
        fov_octant_##p1(&data, 1, TO_20_12(0.0f), end_slope);         \
        fov_octant_##p2(&data, 1, TO_20_12(0.0f), end_slope);         \
        if (a > TO_20_12(1.0f)) { /* a > 1.0f */        \
            start_slope = betweenf(TO_20_12(2.0f) - a, 0, TO_20_12(1.0f));   \
            fov_octant_##p3(&data, 1, start_slope, TO_20_12(1.0f));   \
            fov_octant_##p4(&data, 1, start_slope, TO_20_12(1.0f));   \
        }                                                   \
        if (a > TO_20_12(2.0f)) { /* a > 2.0f */        \
            end_slope = betweenf(a - TO_20_12(2.0f), 0, TO_20_12(1.0f));     \
            fov_octant_##p5(&data, 1, 0, end_slope);     \
            fov_octant_##p6(&data, 1, 0, end_slope);     \
        }                                                   \
        if (a > TO_20_12(3.0f)) { /* a > 3.0f */        \
            start_slope = betweenf(TO_20_12(4.0f) - a, 0, TO_20_12(1.0f));   \
            fov_octant_##p7(&data, 1, start_slope, TO_20_12(1.0f));   \
            fov_octant_##p8(&data, 1, start_slope, TO_20_12(1.0f));   \
        }                                                   \
    }

#define BEAM_DIRECTION_DIAG(d, p1, p2, p3, p4, p5, p6, p7, p8)  \
    if (direction == d) {                                       \
        start_slope = betweenf(TO_20_12(1.0f) - a, 0, TO_20_12(1.0f));           \
        fov_octant_##p1(&data, 1, start_slope, TO_20_12(1.0f));           \
        fov_octant_##p2(&data, 1, start_slope, TO_20_12(1.0f));           \
        if (a > TO_20_12(1.0f)) { /* a > 1.0f */            \
            end_slope = betweenf(a - TO_20_12(1.0f), 0, TO_20_12(1.0f));         \
            fov_octant_##p3(&data, 1, 0, end_slope);         \
            fov_octant_##p4(&data, 1, 0, end_slope);         \
        }                                                       \
        if (a > TO_20_12(2.0f)) { /* a > 2.0f */            \
            start_slope = betweenf(TO_20_12(3.0f) - a, 0, TO_20_12(1.0f));       \
            fov_octant_##p5(&data, 1, start_slope, TO_20_12(1.0f));       \
            fov_octant_##p6(&data, 1, start_slope, TO_20_12(1.0f));       \
        }                                                       \
        if (a > TO_20_12(3.0f)) { /* a > 3.0f */            \
            end_slope = betweenf(a - TO_20_12(3.0f), 0, TO_20_12(1.0f));         \
            fov_octant_##p7(&data, 1, 0, end_slope);         \
            fov_octant_##p8(&data, 1, 0, end_slope);         \
        }                                                       \
    }
